...
export function AnalyticsCard(props) {
  ...
  const targetDate = new Date(siteConfig('RHZ_SITE_CREATE_TIME', null, CONFIG))
  const today = new Date()
  const diffTime = today.getTime() - targetDate.getTime()
  const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24))
  const postCountTitle = siteConfig('RHZ_POST_COUNT_TITLE', null, CONFIG)
  const siteTimeTitle = siteConfig('RHZ_SITE_TIME_TITLE', null, CONFIG)
  const siteVisitTitle = siteConfig('RHZ_SITE_VISIT_TITLE', null, CONFIG)
  const siteVisitorTitle = siteConfig('RHZ_SITE_VISITOR_TITLE', null, CONFIG)
