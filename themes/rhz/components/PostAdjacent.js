...
const adjacentCardClassName =
  'rhz-post-adjacent-card heo-post-adjacent-card group relative flex min-h-[11.5rem] overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white/96 p-5 text-slate-700 shadow-[0_15px_36px_rgba(15,23,42,0.055)] transition-all duration-300 hover:-translate-y-0.5 dark:border-slate-700/55 dark:bg-[#1f2026] dark:text-slate-100'
...
function AdjacentInlineCard({ href, label, title, align = 'left' }) {
  ...
  return (
    <SmartLink href={href} passHref className={`${adjacentInlineCardClassName} ${adjacentFocusClassName} ${isRight ? 'text-right' : ''}`}>
      ...
      {isRight && <i className='fas fa-chevron-right ml-3 shrink-0 text-[0.7rem] text-slate-300 transition-colors duration-300 group-hover:text-slate-500 group-focus:text-blue-500 group-focus-visible:text-blue-500 group-active:text-blue-500 dark:text-slate-600 dark:group-focus:text-orange-300 dark:group-focus-visible:text-orange-300 dark:group-active:text-orange-300 dark:group-hover:text-slate-400' />}
    </SmartLink>
  )
}
...
function AdjacentCard({ href, label, title, cover, align = 'left' }) {
  ...
  return (
    <SmartLink href={href} passHref className={`${adjacentCardClassName} ${adjacentFocusClassName}`}>
      <div className={`relative z-10 flex min-w-0 flex-1 flex-col justify-between ${cover ? (isRight ? 'text-right lg:pr-[7.5rem]' : 'lg:pl-[7.5rem]') : isRight ? 'text-right' : ''}`}>
        ...
        <div className='space-y-3'>
          <div className='rhz-post-adjacent-card__title heo-post-adjacent-card__title line-clamp-2 text-xl font-semibold leading-snug text-slate-800 transition-colors duration-300 group-hover:text-slate-900 group-focus:text-blue-700 group-focus-visible:text-blue-700 group-active:text-blue-700 dark:text-slate-100 dark:group-hover:text-white dark:group-focus:text-orange-200 dark:group-focus-visible:text-orange-200 dark:group-active:text-orange-200'>
            {title}
          </div>
          <div className={`rhz-post-adjacent-card__cta heo-post-adjacent-card__cta flex items-center gap-2 text-sm text-slate-500 transition-all duration-300 group-hover:text-slate-700 group-focus:text-blue-600 group-focus-visible:text-blue-600 dark:text-slate-400 dark:group-hover:text-slate-300 dark:group-focus:text-orange-300 dark:group-focus-visible:text-orange-300 ${isRight ? 'justify-end' : ''}`}>
            <span>继续阅读</span>
            <i className={`fas ${isRight ? 'fa-arrow-up-right-from-square' : 'fa-arrow-right'} text-xs`} />
          </div>
        </div>
      </div>
      {cover && (
        <div className={`pointer-events-none absolute top-4 z-0 hidden h-[5.5rem] w-[6rem] overflow-hidden rounded-[1.25rem] border border-white/22 bg-slate-100/58 backdrop-blur-[2px] dark:border-white/6 dark:bg-slate-800/30 lg:block ${isRight ? 'right-4' : 'left-4'}`}>
          <LazyImage src={cover} className='absolute inset-0 h-full w-full object-cover object-center opacity-62 saturate-[0.86] transition-transform duration-500 group-hover:scale-105' />
...
export default function PostAdjacent({ prev, next }) {
  ...
  if (!siteConfig('RHZ_ARTICLE_ADJACENT', null, CONFIG) || (!prev && !next)) {
    return <></>
  }
  ...
  const adjacentItems = useMemo(
    () => [
      prev && { href: `/${prev.slug}`, label: prevLabel, title: prev.title, cover: prev?.pageCoverThumbnail || prev?.pageCover || null, align: 'left' },
      next && { href: `/${next.slug}`, label: nextLabel, title: next.title, cover: next?.pageCoverThumbnail || next?.pageCover || null, align: 'right' }
    ].filter(Boolean),
    [next, nextLabel, prev, prevLabel]
  )

  const sectionTitle = adjacentItems.length > 1 ? '继续阅读下一篇内容' : next ? '继续阅读下一篇内容' : '回看上一篇内容'

  return (
    <section id='article-end' className='rhz-post-footer__adjacent heo-post-footer__adjacent space-y-4'>
      <div className='px-1'>
        <div className='text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500'>Continue Reading</div>
        <div className='mt-1 text-[1.7rem] font-semibold leading-tight text-slate-800 dark:text-slate-100 sm:text-2xl'>{sectionTitle}</div>
      </div>
      <div className={`grid gap-3 lg:hidden ${adjacentItems.length > 1 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'}`}>
        {adjacentItems.map(item => <AdjacentInlineCard key={item.href} href={item.href} label={item.label} title={item.title} align={item.align} />)}
      </div>
      <div className={`hidden gap-4 lg:grid ${adjacentItems.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
        {adjacentItems.map(item => <AdjacentCard key={item.href} href={item.href} label={item.label} title={item.title} cover={item.cover} align={item.align} />)}
      </div>
      {next && (
        <div id='pc-next-post' aria-hidden={!isShow} className={`${isShow ? 'opacity-[0.9] pointer-events-auto' : 'opacity-0 pointer-events-none'} hidden xl:block fixed z-40 right-8 bottom-6 transition-opacity duration-300`}>
          <SmartLink
            href={`/${next.slug}`}
            ...
            {next?.pageCoverThumbnail || next?.pageCover ? (
              <div className='pointer-events-none absolute right-4 top-4 h-[4.5rem] w-[5rem] overflow-hidden rounded-[1.1rem] border border-white/22 bg-slate-100/58 backdrop-blur-[2px] dark:border-white/6 dark:bg-slate-800/30'>
                <LazyImage src={next?.pageCoverThumbnail || next?.pageCover} className='absolute inset-0 h-full w-full object-cover object-center opacity-56 saturate-[0.82] transition-transform duration-500 group-hover:scale-[1.03]' />
                <div className='absolute inset-0 bg-gradient-to-br from-white/52 via-white/24 to-white/34 dark:from-[#1f2026]/30 dark:via-transparent dark:to-[#1f2026]/26' />
              </div>
            ) : null}
