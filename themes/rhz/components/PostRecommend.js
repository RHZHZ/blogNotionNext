...
export default function PostRecommend({ recommendPosts, siteInfo }) {
  ...
  if (!siteConfig('RHZ_ARTICLE_RECOMMEND', null, CONFIG) || !recommendPosts || recommendPosts.length === 0) {
    return <></>
  }

  return (
    <section className='rhz-post-footer__recommend heo-post-footer__recommend hidden md:block'>
      <div className='overflow-hidden rounded-[1.9rem] border border-slate-200/80 bg-white/96 p-4 shadow-[0_14px_36px_rgba(15,23,42,0.045)] dark:border-slate-700/60 dark:bg-[#1f2026] sm:p-5'>
        <div className='mb-4 flex flex-col gap-2 px-1 sm:mb-5 sm:flex-row sm:items-end sm:justify-between sm:gap-4'>
          <div>
            <div className='text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500'>Related Posts</div>
            <div className='mt-1 text-2xl font-semibold text-slate-800 dark:text-slate-100'>{locale.COMMON.RELATE_POSTS}</div>
          </div>
          <div className='text-sm text-slate-400 dark:text-slate-500'>为你继续延伸阅读线索</div>
        </div>
            ...
            return (
              <SmartLink
                ...
                <div className='relative h-28 overflow-hidden border-b border-slate-200/65 dark:border-slate-700/55 sm:h-32'>
                  <LazyImage src={headerImage} className='absolute inset-0 h-full w-full object-cover object-center transition-transform duration-500 group-hover:scale-105' />
