...
export { fetchNotionPageBlocks } from './notion/getPostBlocks'

/**
 * 清洗 blockMap，确保所有 ID 引用都是字符串，防止 notion-utils uuidToId 崩溃
 */
function cleanBlockMap(blockMap) {
  if (!blockMap?.block) {
    return blockMap
  }
  ...
  Object.keys(blockMap.block).forEach(key => {
    ...
    const b = blockMap.block[key]
    if (!b?.value) {
      return
    }

    b.value.id = String(b.value.id || key)

    if (b.value.parent_id === undefined || b.value.parent_id === null) {
      b.value.parent_id = ''
    } else {
      b.value.parent_id = String(b.value.parent_id)
    }

    if (Array.isArray(b.value.content)) {
      b.value.content = b.value.content
        .filter(id => id !== undefined && id !== null && id !== 'undefined' && id !== 'null')
        .map(id => String(id))
    }

    if (Array.isArray(b.value.children)) {
      b.value.children = b.value.children.filter(Boolean)
    }
 ...
 * @param {*} slice
 */
export async function getPostBlocks(id, from = null, slice = 0) {
  const blockMap = await fetchNotionPageBlocks(id, from, slice)
  if (!blockMap) {
    return null
  }

  // 兼容 Notion 数据格式
  let adaptedBlockMap = adapterNotionBlockMap(blockMap)
  // 深度清洗 blockMap，防止 notion-utils uuidToId 崩溃
  adaptedBlockMap = cleanBlockMap(adaptedBlockMap)
  // 格式化内容
  adaptedBlockMap = {
    ...adaptedBlockMap,
...
export async function resolvePostProps({
  ...
  if (post && post.id && !post?.blockMap) {
    try {
      // 使用 getPostBlocks 自动处理兼容、清洗和格式化
      post.blockMap = await getPostBlocks(post.id, source)
    } catch (e) {
      console.warn('[resolvePostProps] getPostBlocks failed:', post.id, e)
    }
  }
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  pageRecordMap = adapterNotionBlockMap(pageRecordMap)
  pageRecordMap = cleanBlockMap(pageRecordMap)
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[SiteDataApi] pageIds summary', {
      collectionId,
      viewIds,
      pageIdsCount: pageIds?.length || 0,
      samplePageIds: pageIds?.slice?.(0, 10) || []
    })
  }
  ...
  block = cleanBlockMap({ block: Object.assign({}, block, adaptedFetchedBlocks) }).block
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (pageBlock.parent_id !== collectionId) {
      continue
    }
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(
      '[SiteDataApi] collectionData sample',
