...
export { getAllTags } from './notion/getTags'
  ...
  Object.keys(blockMap.block).forEach(key => {
    if (key && key !== 'undefined' && key !== 'null') {
      ...
      if (b?.value) {
        ...
        // 修复 parent_id
        if (b.value.parent_id === undefined) b.value.parent_id = ''
        // 修复 content 数组中的 undefined
        if (Array.isArray(b.value.content)) {
          b.value.content = b.value.content.filter(id => id !== undefined && id !== null)
        }
        cleanBlock[key] = b
      }
    }
