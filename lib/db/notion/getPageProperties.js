...
const getCheckboxValue = val => {
  ...
  if (Array.isArray(val)) {
    const flatValues = val.flat(Infinity)
    for (const item of flatValues) {
...
export default async function getPageProperties(
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      ...
      switch (schema[key]?.type) {
        ...
        case 'checkbox': {
          const checkboxValue = getCheckboxValue(val)
          if (typeof checkboxValue === 'boolean') {
            properties[schema[key].name] = checkboxValue
          }
          break
        }
        default:
  ...
  const titlePropertyName = Object.entries(schema || {}).find(([, item]) => item?.type === 'title')?.[1]?.name

  // 将 Notion 标题字段规范化为纯文本，避免 rich text 中的换行传到文章头部
  if (titlePropertyName && properties[titlePropertyName]) {
    properties[titlePropertyName] = normalizePlainText(properties[titlePropertyName])
  }
