...
const uploadBufferToCos = async ({ objectKey, buffer, contentType }) => {
  ...
  return `${COS_PUBLIC_BASE_URL}/${objectKey}`
}

const writeArchiveTarget = async ({ filename, buffer, contentType }) => {
  if (hasCosStorageConfigured()) {
    const objectKey = `${COS_KEY_PREFIX}/${filename}`.replace(/^\/+/, '')
    const archivedAudioUrl = await uploadBufferToCos({ objectKey, buffer, contentType })
    return {
      archivedAudioUrl,
      archivePath: `cos://${COS_BUCKET}/${objectKey}`,
      storageType: 'cos'
    }
  }
  ...
  return {
    archivedAudioUrl: `${PUBLIC_BASE_URL}/${filename}`,
    archivePath,
    storageType: 'local'
  }
}
...
const archiveAudioFile = async ({ page, sourceUrl, title, trackId, dryRun }) => {
  ...
  if (dryRun) {
    ...
    return {
      archivedAudioUrl: `${PUBLIC_BASE_URL}/${filename}`,
      archivePath: hasCosStorageConfigured()
        ? `cos://${COS_BUCKET}/${COS_KEY_PREFIX}/${filename}`
        : path.join(ARCHIVE_DIR, filename),
      finalUrl: sourceUrl,
      size: 0,
      downloadedFrom: sourceUrl,
      downloadProfile: 'dry-run',
      storageType: hasCosStorageConfigured() ? 'cos' : 'local'
    }
  }
...
async function runArchiveTask(customOptions = null) {
  ...
  console.log(`Archive Storage: ${hasCosStorageConfigured() ? 'tencent-cos' : 'local'}`)
  ...
  if (inputPlaylistId) {
    ...
    for (const [index, trackMeta] of playlistTracks.entries()) {
      ...
      try {
        ...
        console.log(`  存储类型: ${archiveResult.storageType}`)
        if (!options.dryRun) {
        ...
        if (!options.dryRun) {
          await updateArchivePage({
            ...
            values: {
              ...
              album,
              trackId,
              storageProvider: hasCosStorageConfigured() ? 'cos' : 'local',
              storageKey: '',
              fileSize: '',
              sourceType: 'meting-playlist'
            }
