...
async function enrichBooksWithDetails(books = [], cookie) {
  ...
  if (!needsDetail.length) return normalizedBooks
  ...
  return normalizedBooks.map(book => {
    ...
    return detail
      ? {
          ...
          href: book?.href || detail?.href || buildHref(bookId, detail?.infoId)
        }
      : book
  })
}
...
const mapShelfBook = book => {
  ...
  return {
    ...
    note: bookId ? `微信读书 ID：${bookId}` : '',
    href: buildHref(bookId, infoId) || normalizeText(book?.href)
  }
}
...
function buildBookListSnapshot(shelfSnapshot = {}) {
  ...
  return {
    ...
    books: uniqueByBookId(sections.flatMap(section => section.books))
  }
}
...
function parseNotionSnapshotPages(pages = []) {
  ...
  const sortedBookListBooks = bookListBooks.sort(sorter)
  const archiveMap = new Map()
  for (const book of sortedBookListBooks) {
...
async function upsertNotionSnapshot(snapshot, databaseId) {
  ...
  const groups = [
    ...
    ['booklist', uniqueByBookId(snapshot.bookList?.books || []), true]
  ]
...
export async function syncWereadAboutBooks(options = {}) {
  ...
  try {
    ...
    selected = {
      favoriteBooks: await enrichBooksWithDetails(selected.favoriteBooks, cookie),
      recentBooks: await enrichBooksWithDetails(selected.recentBooks, cookie)
    }
    const selectedBookList = buildBookListSelection({
      shelfSnapshot: {
        ...shelfSnapshot,
        mergedBooks: await enrichBooksWithDetails(shelfSnapshot.mergedBooks, cookie),
        lectureBooks: await enrichBooksWithDetails(shelfSnapshot.lectureBooks, cookie)
      },
      config: {
        ...config,
        bookListShelfNames: config.bookListShelfNames
