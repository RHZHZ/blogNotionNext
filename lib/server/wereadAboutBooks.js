...
const dedupeBookIds = (bookIds = []) => {
  const result = []
  const seen = new Set()

  for (const bookId of Array.isArray(bookIds) ? bookIds : []) {
    const normalizedBookId = normalizeBookId(bookId)
    if (!normalizedBookId || seen.has(normalizedBookId)) continue
    seen.add(normalizedBookId)
    result.push(normalizedBookId)
  }

  return result
}

const buildBookListRelationStats = (books = []) => {
  const shelfMap = new Map()
  let emptyShelfNameCount = 0
  ...
  for (const book of Array.isArray(books) ? books : []) {
    ...
    if (!bookId) continue
    if (!shelfName) {
      emptyShelfNameCount += 1
      continue
    }
    const current = shelfMap.get(bookId) || { shelfNames: new Set(), title: '', href: '' }
    current.shelfNames.add(shelfName)
    current.title = current.title || normalizeText(book?.title)
    current.href = current.href || normalizeText(book?.href)
    shelfMap.set(bookId, current)
  }

  const multiShelfEntries = Array.from(shelfMap.entries())
    .map(([bookId, value]) => ({
      bookId,
      title: value.title,
      href: value.href,
      shelfNames: Array.from(value.shelfNames).sort((a, b) => a.localeCompare(b, 'zh-Hans-CN'))
    }))
    .filter(item => item.shelfNames.length > 1)
    .sort((a, b) => b.shelfNames.length - a.shelfNames.length || a.bookId.localeCompare(b.bookId))

  return {
    relationCount: Array.isArray(books) ? books.length : 0,
    uniqueBookCount: shelfMap.size,
    multiShelfBookCount: multiShelfEntries.length,
    multiShelfEntries: multiShelfEntries.slice(0, 20),
    emptyShelfNameCount
  }
}
...
const buildNotionSnapshotStats = pages => {
  ...
  for (const page of allPages) {
    ...
    if (enabled) enabledPages.push(page)
    else disabledPages.push(page)
  }

  const { duplicates } = buildNotionIndex(allPages)
  let duplicatePageCount = 0
  let duplicateBooklistKeyCount = 0
  let duplicateNonBooklistKeyCount = 0

  for (const [key, pagesOfSameKey] of duplicates.entries()) {
    duplicatePageCount += pagesOfSameKey.length
    if (String(key || '').startsWith('booklist:')) duplicateBooklistKeyCount += 1
    else duplicateNonBooklistKeyCount += 1
  }


  const countByGroup = pages => {
    return pages.reduce(
      (acc, page) => {
        const properties = page?.properties || {}
        const group = normalizeText(readPlainText(pickProperty(properties, PROPERTY_CANDIDATES.group))).toLowerCase()
        if (group === 'booklist') acc.booklist += 1
        else acc.nonBooklist += 1
        return acc
      },
      { booklist: 0, nonBooklist: 0 }
    )
  }

  const enabledGroupCounts = countByGroup(enabledPages)
  const disabledGroupCounts = countByGroup(disabledPages)

  return {
    totalPages: allPages.length,
    enabledPageCount: enabledPages.length,
    disabledPageCount: disabledPages.length,
    enabledBooklistPageCount: enabledGroupCounts.booklist,
    enabledNonBooklistPageCount: enabledGroupCounts.nonBooklist,
    disabledBooklistPageCount: disabledGroupCounts.booklist,
    disabledNonBooklistPageCount: disabledGroupCounts.nonBooklist,
    duplicateBooklistKeyCount,
    duplicateNonBooklistKeyCount,
...
function buildBookListSnapshot(shelfSnapshot = {}) {
  ...
  return {
    archive: sections.map(section => ({
      ...
      bookIds: dedupeBookIds(section.books.map(book => normalizeBookId(book?.bookId)).filter(Boolean))
...
export async function syncWereadAboutBooks(options = {}) {
  ...
  try {
    ...
    const notionSnapshotStats = config.notionDbId && process.env.NOTION_ACCESS_TOKEN
      ? buildNotionSnapshotStats(await queryNotionDatabase(config.notionDbId))
      : null
    const bookListRelationStats = buildBookListRelationStats(bookList.books)
    syncDebug = {
      ...buildShelfDebug({
        shelfSnapshot,
        favoriteShelfNames: selectedFavoriteShelfNames,
