...
export async function processPostData(props, from) {
  ...
  if (props.post?.blockMap?.block) {
    ...
    const hasAiSummaryField = Object.prototype.hasOwnProperty.call(
      props.post || {},
      'ai_summary'
    )
    const canUseManualSummaryAsAiSummary = hasAiSummaryField && aiSummaryEnabled

    if (!aiSummaryEnabled) {
      props.post.aiSummary = null
    } else if (!props.post.summary && !props.post.aiSummary) {
    ...
    } else if (canUseManualSummaryAsAiSummary && props.post.summary && !props.post.aiSummary) {
      // 仅在显式开启 AI 摘要时，允许使用 Notion 的 summary 作为摘要展示内容
