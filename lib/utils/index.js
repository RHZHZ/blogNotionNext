...
/**
 * 当前文章是否启用 AI 摘要
 * 支持 Notion 属性 `ai_summary` 作为开关
 * 以下值视为关闭：false / 0 / off / no
 * @param {*} post
 * @returns {boolean}
 */
export function isAiSummaryEnabled(post) {
  if (!post || typeof post !== 'object') return true

  const hasAiSummaryField = Object.prototype.hasOwnProperty.call(post, 'ai_summary')
  if (!hasAiSummaryField) return true

  const value = post.ai_summary
  ...
  if (Array.isArray(value)) {
    ...
    return isAiSummaryEnabled({ ai_summary: value[0] })
  }

  const normalized = String(value).trim().toLowerCase()
  if (!normalized) return true

  return !['false', '0', 'off', 'no', 'unchecked', 'null', 'undefined'].includes(normalized)
}

/**
 * AI 摘要是否为可展示的有效内容
 * 失败、繁忙、配置错误等提示文案不应被展示或缓存
 * @param {string} summary
 * @returns {boolean}
 */
export function isValidAiSummary(summary) {
