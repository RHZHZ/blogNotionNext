...
import { getGlobalContextSnapshot } from './global-context'
...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  switch (key) {
    ...
    case 'AI_SUMMARY_SEGMENT_LIMIT':
    case 'AI_SUMMARY_TOC_LIMIT':
    case 'AI_SUMMARY_MIN_LENGTH':
    case 'AI_SUMMARY_LENGTH_SHORT':
    case 'AI_SUMMARY_LENGTH_MEDIUM':
    case 'AI_SUMMARY_LENGTH_LONG':
    case 'AI_SUMMARY_LENGTH_XLONG':
    case 'UUID_REDIRECT':
      // LINK比较特殊，
      if (key === 'LINK') {
  ...
  let global = {}
  try {
    global = getGlobalContextSnapshot()
  } catch (error) {
    // 本地调试用
