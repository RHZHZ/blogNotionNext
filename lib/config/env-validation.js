...
const ENV_VALIDATION_RULES = {
  ...
  security: {
    REDIS_URL: {
      ...
      description: 'Redis 连接地址，用于共享缓存与限流 provider'
    },
    UPSTASH_REDIS_REST_URL: {
      validator: (value) => {
        if (value && !Validator.isValidUrl(value)) {
          return 'UPSTASH_REDIS_REST_URL must be a valid URL'
        }
        return true
      },
      description: 'Upstash REST API 地址，与 UPSTASH_REDIS_REST_TOKEN 配套使用'
    },
    UPSTASH_REDIS_REST_TOKEN: {
      minLength: 1,
      description: 'Upstash REST API Token，与 UPSTASH_REDIS_REST_URL 配套使用'
    },
    KV_REST_API_URL: {
      validator: (value) => {
        if (value && !Validator.isValidUrl(value)) {
          return 'KV_REST_API_URL must be a valid URL'
        }
        return true
      },
      description: 'KV REST API 地址，与 KV_REST_API_TOKEN 配套使用'
    },
    KV_REST_API_TOKEN: {
      minLength: 1,
      description: 'KV REST API Token，与 KV_REST_API_URL 配套使用'
    },
...
function validatePairedEnvVars(leftKey, rightKey, errors) {
  const left = process.env[leftKey]
  const right = process.env[rightKey]

  if ((left && !right) || (!left && right)) {
    errors.push(`${leftKey} and ${rightKey} must be configured together`)
  }
}
...
export function validateEnvironmentVariables() {
  ...
  validatePairedEnvVars('UPSTASH_REDIS_REST_URL', 'UPSTASH_REDIS_REST_TOKEN', errors)
  validatePairedEnvVars('KV_REST_API_URL', 'KV_REST_API_TOKEN', errors)
